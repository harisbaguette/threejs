...
export function createCharacterPreview(canvas, player, environment) {
  ...
  const scene = new THREE.Scene(); scene.environment = environment; scene.environmentIntensity = .9;
  ...
  const hemisphere = new THREE.HemisphereLight('#e9f4ff', '#887568', 1.65); scene.add(hemisphere);
  const key = new THREE.DirectionalLight('#fff0dc', 3.2); key.position.set(2, 3, 4); scene.add(key);
  const fill = new THREE.DirectionalLight('#c7e4ff', 1); fill.position.set(-3, 2, 2); scene.add(fill);
  ...
  function frameView() {
    ...
    if (view === 'face') {
      ...
      camera.position.set(.15, height * .59, 3.45);
  ...
  return {
    ...
    update(dt) {
      ...
      current?.mixer.update(dt); controls.update(); renderer.render(scene, camera);
    },
  };
}
