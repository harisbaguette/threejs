...
export const CHARACTERS = [
  { id: 'female', label: '여성', name: '서연', height: 1.69, url: '/assets/Alina.glb' },
  { id: 'male', label: '남성', name: '도윤', height: 1.8, url: '/assets/Traveler.glb' },
];
...
function prepareMaterials(model, female) {
  model.traverse(obj => {
    ...
    if (female) {
      ...
      } else if (/eyelash|hair|skullcap/.test(name)) {
        material.transparent = true; material.alphaTest = .04; material.depthWrite = false;
        material.roughness = .55; material.forceSinglePass = true;
        obj.renderOrder = name.includes('skullcap') ? 1 : 2;
...
export async function loadCharacters(manager) {
  ...
  female.scene.traverse(mesh => {
    if (!mesh.isMesh) return;
    // Preserve the public asset's skin, hair, clothing and authored silhouette.
  });
  const femaleClips = [];
